...
- Support English and Simplified Chinese, long translated labels, display scaling and smaller windows without clipped actions or unreadable information.
...
整个原生 Qt 控制面板统一采用 Material 3 / Material You 设计，覆盖所有页面、明暗主题、个性化强调色、中英文、缩放、键盘操作与异常状态。该项属于最终交付要求，不替代完整 QEMU 固件模式；原厂 Tesla 车机界面本身不做这次视觉改造。已实现共享主题、原生组件与七页应用外壳，并检查 Windows / Debian WSLg 原生截图；0.5.0 已提供 QEMU 运行状态截图；完整功能等效的剩余项目见虚拟桌面说明。
...
- Captured all seven pages in light/dark appearance and English/Simplified Chinese on Windows Qt and Debian WSLg.
