...
try:
    ...
    window.refresh_preview()
    wait_until(lambda: not window.overview.center.isNull())
    window.grab().save(str(args.destination / 'workspace-rebuilt.png'))
    ...
    print('Native UI: display previews, controls, both display acknowledgements, camera window passed. Lifecycle cycle:', args.cycle, 'Display tools:', args.tools)
finally:
    window.show_pattern()
